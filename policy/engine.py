...
def decide(rule_results: list[RuleResult], confidence: float, dry_run: bool = True) -> PolicyDecision:
    ...
    Thresholds (deliberately simple; tuned only on the dev set, never
    the held-out set):
    - 0 rules fired -> no_action
    - >=1 rule fired, confidence < 0.5 -> flag_for_review (human looks, nothing blocked)
    - >=1 rule fired, 0.5 <= confidence < 0.8 -> hold_for_verification (extra step, not a hard block)
    - >=2 rules fired OR confidence >= 0.8 -> soft_decline (the strongest bounded action available; never a silent hard block)
    """
    ...
    elif len(fired) >= 2 or confidence >= 0.8:
